...
def load_lidc_variants(input_dir):
    ...
    for fname in os.listdir(input_dir):
        ...
        if dataset_root != TARGET_ROOT or uq_method not in UQ_METHODS:
            continue
        try:
...
def plot_2d_projections(data, uq_method, color_map):
    ...
    out_path = os.path.join(OUTPUT_DIR, f"spatial_fingerprint_2d_lidc_{uq_method}.png")
...
def plot_3d_interactive(data, uq_method, color_map):
    ...
    out_path = os.path.join(OUTPUT_DIR, f"spatial_fingerprint_3d_lidc_{uq_method}.html")
