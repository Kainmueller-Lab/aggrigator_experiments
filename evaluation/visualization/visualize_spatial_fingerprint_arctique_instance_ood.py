...
def load_arctique_variants(input_dir):
    """Load CSVs from all arctique* datasets only"""
    ...
    for fname in os.listdir(input_dir):
        ...
        if not "arctique_instance" in dataset_name:
            continue
        try:
...
def plot_2d_projections(data, uq_method, color_map):
    ...
    out_path = os.path.join(OUTPUT_DIR, f"spatial_fingerprint_2d_arctique_nuclei_intensity_ood_{uq_method}.png")
...
def plot_3d_interactive(data, uq_method, color_map):
    ...
    fig.update_layout(
        title=f"3D Spatial Fingerprints (arctique, {uq_method})",
        scene=dict(
    ...
    out_path = os.path.join(OUTPUT_DIR, f"spatial_fingerprint_3d_arctique_nuclei_intensity_ood_{uq_method}.html")
    fig.write_html(out_path)
    print(f"3D interactive plot saved to: {out_path}")


if __name__ == "__main__":
    data = load_arctique_variants(INPUT_DIR)
    all_datasets = sorted(set(ds for uq in UQ_METHODS for ds in data[uq].keys()))
    if not all_datasets:
        print("No arctique datasets found.")
    else:
