...
def load_arctique_variants(input_dir):
    ...
    for fname in os.listdir(input_dir):
        ...
        if not "arctique_semantic" in dataset_name:
            continue
        try:
...
def plot_2d_projections(data, uq_method, color_map):
    ...
    out_path = os.path.join(OUTPUT_DIR, f"spatial_fingerprint_2d_arctique_blood_cells_ood_{uq_method}.png")
...
def plot_3d_interactive(data, uq_method, color_map):
    ...
    out_path = os.path.join(OUTPUT_DIR, f"spatial_fingerprint_3d_arctique_blood_cells_ood_{uq_method}.html")
