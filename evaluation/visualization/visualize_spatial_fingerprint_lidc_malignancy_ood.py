...
def load_lidc_variants(input_dir):
    """Load CSVs from all lidc* datasets only"""
    ...
    for fname in os.listdir(input_dir):
        ...
        if not "lidc_fgbg_malignancy" in dataset_name:
            continue
        try:
...
def plot_2d_projections(data, uq_method, color_map):
    ...
    out_path = os.path.join(OUTPUT_DIR, f"spatial_fingerprint_2d_lidc_malignancy_ood_{uq_method}.png")
...
def plot_3d_interactive(data, uq_method, color_map):
    ...
    fig.update_layout(
        title=f"3D Spatial Fingerprints (lidc, {uq_method})",
        scene=dict(
    ...
    out_path = os.path.join(OUTPUT_DIR, f"spatial_fingerprint_3d_lidc_malignancy_ood_{uq_method}.html")
    fig.write_html(out_path)
    print(f"3D interactive plot saved to: {out_path}")


if __name__ == "__main__":
    data = load_lidc_variants(INPUT_DIR)
    all_datasets = sorted(set(ds for uq in UQ_METHODS for ds in data[uq].keys()))
    if not all_datasets:
        print("No lidc datasets found.")
    else:
