...
class TestSummary(unittest.TestCase):
    def test_uncertainty_map_classes(self):
        ...
        array = mnist_dataset[0][0].numpy()
        mask = (mnist_dataset[0][0].numpy() > 0).astype(int)
        name = f"UncMap{0}"
    ...
    def test_results(self):
        ...
        arrays = [mnist_dataset[index][0].numpy() for index in range(N)]
        masks = [(mnist_dataset[index][0].numpy() > 0).astype(int) for index in range(N)]
